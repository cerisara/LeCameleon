...
sreenivas2024minitron}, which uses over 50B tokens for compression. Sample efficiency is an important property in post-training compression, as it
...
Compared to finegrained sparse mixture-of-experts and sparse memory layers,
ZoN get rid of the TopK and makes point-wise sparsity prediction, which
make it not only more compute-adaptive but also more communication efficient
...
it. Maybe this will not be a problem in few years with the next generation
of hardware where a single GPU with TB of vRAM that can train
multi-trillion parameters LLMs on a single GPU. 
