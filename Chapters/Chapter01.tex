%************************************************
\chapter{Modeling the Language}\label{ch:introduction}
%************************************************
\section{A dialog between Shannon and Chomsky}
It is curious that we still don't know where the expression \textit{language model} come from and when it etablishes. 
Not the notion of what is a \textit{language model} but the \textit{expression} itself. 
Because for the notion, we know that the idea of language model is quite old and one of the earliest 
...
\begin{quote}
    ...
    \end{flushright}
\end{quote}
Shannon, meanwhile, describes a statistical 
model of English but, importantly, did not call it \textit{language models}.
...
Calling these systems as such could suggest that they are
a linguistic model of a language. But does a better statistical model of English mean a better model of this language?
The idea that statistical learning can capture linguistic structure is compatible 
with usage-based and connectionist approaches. Nativist approaches, 
...
designing learning architectures that optimizes the language modeling objective, which results
in profiency linguistic competence of such systems.

The language modeling objective, as Shannon describes it, is learning to prectict
...
The function $\mathrm{score}(x, y)$ can be designed very freely but in neural language model, it is often the exponential of the dot product
...
While \citep{bahdanau2015neural} improved neural machine translation by adding attention
to an RNN-based encoder--decoder, \citep{vaswani2017attention} proposed replacing recurrence
with attention in the Transformer. Removing recurrent dependencies allows all token positions
within a layer to be processed in parallel during training, making the architecture more
parallelisable and easier to scale. Figure~\ref{fig:decoder-only-transformer} presents the
...
The innovation of Transformer is to remove the RNN and replace it fully with
...
Having multiple independant heads is beneficial since it has been shown
...
Citing \citep{shazeer2020gluvariants}, the author of gated MLPs:
\begin{quote}
    \small\itshape
...
\citep{geva-etal-2021-transformer} interpret the MLP as a key--value memory:
...
The LM Head weight is another important component of large-scale Transformer LLMs. It is closely linked to the tokenizer: 
...
Recent tokenizers use a large vocabulary due to the diversity of tokens that can be infered from a web scale data. 
Hence, the LM Head weight can occupy a huge amount of parameters in the LLM, particulary for small LLMs. Figure \ref{fig:emb-param-ratio} shows the memory
...
weight tying \citep{press-wolf-2017-using}, which shares the input and output embedding. Almost all small LLMs use weight tying, which haven't
...
However, this bottlneck architecture can hurt optimization. 
...
model's ability to predict complex token distributions. \citep{godey2024smalllanguagemodelsunderperform}
...
$r \ll V$, an obvious solution is to augment $r$ or reduce $V$ the voabulary size. Reducing the vocabulary size can be
achieved using a tokenizer with few tokens or falling back to a character or bytes tokenizers.
...
crossing a plateau and acquiring a new ability. \citep{wei2022emergent} identify
several such \textit{emergent abilities} as shown in Figure~\ref{fig:wei-emergent-abilities}.
\begin{figure}[ht]
    \centering
...
\end{figure}
We can see that many linguistic skills such as language understanding, specially
...
performance is measured. \citep{sinha2026illusion} argue that short-task
