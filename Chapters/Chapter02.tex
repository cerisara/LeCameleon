...
Our approach proposes to overcome some limitations of the compression approaches
...
To measure inference speed, we measure the \textit{prefell speed}, which is the time to forward a batch of prompts through the entire model and present the results in tokens per second. Using lm-evaluation-harness \citet{eval-harness}, we evaluate zero-shot performance on 9 downstream tasks: TruthfulQA (TruQA) \citet{lin-etal-2022-truthfulqa}, Social IQa (SIQA) \citet{sap2019social}, LogiQA \citet{liu2020logiqa}, WinoGrande (WinoG) \citet{sakaguchi2019winogrande}, ARC Easy (ARC-E) \citet{arc}, ARC Challenge (ARC-C) \citet{arc}, BoolQ \citet{boolq}, PIQA \citet{piqa}, and OpenBookQA (OBQA) \citet{obqa}. We compare non-compressed (0\%) and compressed variants.
...
For most human languages, there is not enough data (generally less than 1B tokens) to pretrain large LLMs. In such cases, data-efficient compression approaches are preferred. We experimented compressing a tiny language model for Hausa, a low-resource language for which there is not billions of tokens available for continued pretraining. We used InkubaLM~\citep{tonja2024inkubalm}, a 422M parameter language model designed for five low-resource languages. Since 60\% of the parameters are in the embeddings, we focused on compressing the input embedding $W_{e}$ and the prediction head $W_{o}$. We will discuss later on the use of low-rank structure in the LM head, as it appears to contradict some empirical observations from previous studies. For now, let's focus on compressing the input and output modules using applying our approach through local distillation of the low-rank embeddings and prediction head:
