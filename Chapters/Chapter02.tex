%*****************************************
\chapter{Lillama}\label{ch:lillama}
%*****************************************
...
SparseGPT \cite{frantar2023sparsegptmassivelanguagemodels} & \xmark & \xmark & \xmark & \cmark & - \\
Wanda \cite{wanda} & \xmark & \xmark & \xmark & \cmark & - \\
Teal \cite{TEAL} & \xmark & \xmark & \xmark & \cmark & - \\
ShearedLLaMA \cite{xia2024sheared} & \cmark & \cmark & \cmark & \xmark & 52B\\ 
Minitron \cite{minitron} & \cmark & \cmark & \cmark & \xmark & 94B \\
...
We discuss next previous compression methods, highlight their limitations, and explain how our approach addresses them. Table~\ref{tab:litt} positions our approach relative to recent compression methods.
\subsection{Pruning}
Pruning methods remove unimportant weights in the pre-trained model \citet{NIPS1989_6c9882bb}, \citet{han2015learning}. Pruning can be applied to models in different ways and there are three pruning families:
\subsubsection{Unstructured Pruning} 
In unstructured pruning, individual parameters are removed, which results in a model with sparse weights. As is, this approach does not provide memory savings or speedup without dedicated algorithmic tooling. Since zeros are placed in pre-trained weights in an unstructured and unpredictable way, it is difficult to exploit this on GPU tensor cores. However, when many parameters are pruned, such that there are many zeros in the weight matrix, tensor multiplication can be accelerated with dedicated GPU kernels. Similarly, some tensor storage formats can provide substantial memory gains for an unstructuredly pruned model.
\subsubsection{Semi-Structured Pruning} 
Contrary to unstructured pruning, semi-structured pruning enforces fixed local sparsity patterns (e.g., N:M), as in \citet{wanda}, \citet{frantar2023sparsegptmassivelanguagemodels}. These approaches often do not lead to memory gains but can speed up processing on kernels optimized for such sparse patterns. On the other hand, our method, which directly shrinks matrices, saves memory across all hardware and leads to speedup, as fewer computations are performed.
\subsubsection{Structured Pruning} Structured pruning removes entire groups of parameters, which results in a smaller and faster model. \citet{ma2023llmpruner} propose a gradient-based criterion to eliminate substructures in LLMs, while \citet{xia2024sheared} use a joint loss combining a pruning mask loss with the language modeling loss. However, optimizing these criteria can be computationally intensive. For example, the pruning step of Sheared-LLaMA \citet{xia_sheared_2023} is 5x expensive compared to standard LM training, according to the authors. In contrast, thanks to the local gradient updates, our approach is computationally efficient, allowing us to compress a 47B model within minutes on a single A100 GPU.

\subsection{Low-Rank Decomposition}
Low-Rank Decomposition compresses a model by approximating its pre-trained matrices with lower-dimension ones. Based on the observation by \citet{li2018measuring} that Neural Networks have lower intrinsic dimensions, \citet{aghajanyan2020intrinsic} show that Transformer language models also require lower intrinsic dimensions depending on the language task. However, \citet{yu_compressing_2023}, \citet{chen_drone} show that the weights in some pretrained Transformer models are full-rank compared to their activations and propose an activation-aware compression method.

Our work is related to these approaches, as we also decompose the matrices of the models. However, we apply a lightweight layer-wise feature distillation objective to better recover from compression and propose different distillation strategies to accelerate convergence and improve performance.

\subsection{Distillation}
In Knowledge Distillation (KD), a small student model learns to reproduce the outputs of a larger pre-trained model. Standard KD is data-intensive when the student model's parameters are randomly initialized. Many recent open-source LLMs such as Gemma-4 or Llama-4, are distilled from a larger model using trillion of tokens, which is as costly as a standard pretraining stage. 

Another approach is layer-wise distillation, which can be more suitable in some cases, as intermediate layers may contain certain information, such as speaker identity or phonemes in pretrained speech models \citet{baevski2020wav2vec20frameworkselfsupervised}, \citet{pasad2022layerwiseanalysisselfsupervisedspeech}, \citet{chang2022distilhubertspeechrepresentationlearning}, \citet{chang2021explorationselfsupervisedpretrainedrepresentations}. For example, TinyBERT \citet{jiao_tinybert_2020} initializes the student model by removing layers from the teacher and then distills the student on task-specific knowledge, combining layer-wise and global distillation losses, which took 3.5 days to train. This is costly for LLMs. Instead of removing layers, we leverage the observation that layer activations are low-rank and approximate them with fewer parameters. Additionally, we compress the models to remain generalist rather than task-specific.

\section{Motivation: Activations are more low-rank than weights}
\label{ssec:srank}
In order to motivate the compression of the the models weights with SVD, the weights should have a low low-rank structure. However, previous studies \citet{chen_drone}, \citet{yu_compressing_2023} have shown that the weights of pretrained Transformer models are often full-rank, while their activations are more low-rank.
We empirically confirm this observation on three recent models: Falcon-Mamba, Mistral-v0.1 7B, and Phi-2 3B. There are different measure for rank. In this study, we use a simpler one: stable rank ($srank$) as a proxy measure for rank. The stable rank of a matrix is defined as the following ratio:
\begin{equation}
...
Figure~\ref{fig:srank} shows the stable rank of various weight matrices and layer activations for three models: Falcon-Mamba, Mistral-v0.1 7B, and Phi-2 3B. For all models, activations are significantly lower-rank than weight matrices, which confirms previously published results by \citet{yu_compressing_2023}.

This does not only motivate the use of activations for compression, but also explains why applying SVD to weights alone does not work well as we will show in the experiments. How to explain the fact that weights are full-rank while activations are low-rank? This is an open question. We know that modern LLMs are pre-trained on large and diverse datasets from the web that represent lot of knowledge. Hence the weights of the model need to be full-rank to capture all this knowledge. However, for a given input of a specific task, only a subset of this knowledge is needed, which may explain the low-rank structure of activations.
...
Before presenting our approach, we first introduce the two methods for low-rank approximation of Transformer models: low-rank approximation from weights and from features.
...
As discussed previously, many studies \citet{chen_drone}, \citet{yu_compressing_2023} have shown that the activations (i.e., features) of pretrained transformers are more low-rank than the weights. Recently, \citet{TEAL} show that transformer activations can be sparsified up to 60\%.
Hencce the motivation to use the activations of the linear layers to estimate the low-rank matrices.
...
\end{equation}
An analytic solution to this minimization problem is the eigendecomposition of the covariance matrix of the activations. We first collect all activations $\mathcal{Y}=\{y=Wx\}_{\forall x\in \mathcal{D}}$, then the covariance matrix of the activations can be estimated as $\Sigma = \underset{y \in \mathcal{Y}}{\mathbb{E}}\left[yy^T\right] - \mathbb{E}[y]\mathbb{E}[y]^T$ with $\Sigma \in \mathbb{R}^{d_{1} \times d_{1}}$. Since $\Sigma$ is a diagonalizable matrix, we can apply its eigendecomposition: $\Sigma = USU^{T}$, where $U \in \mathbb{R}^{d_{1} \times d_1}$ contains the eigenvectors of $\Sigma$ and $S \in \mathbb{R}^{d_{1} \times d_{1}}$ is the diagonal matrix containing the eigenvalues sorted in decreasing order. As for the SVD equation above, we can keep the eigenvectors corresponding to the $r$ largest eigenvalues, which gives us $A = U_{:, :r}$ and $B = U_{:, :r}^{T}W$.

Compared to the low-rank matrices in the SVD equation above, $A$ and $B$ weights here are learned to reproduce the activations of the base weight $W$, thanks to the minimization objective above. In the next section, we highlight the limitations of this approach and introduce our proposed method.
...
\begin{figure}[t]
    ...
    \caption{\textbf{Accelerating convergence through a joint loss}. We compare three distillation strategies: (a) Teacher, (b) Student, and (c) Teacher+Student.}
...
The previous analytical solution above is limited to activations from linear layers.
...
\end{equation}
where the Student module is parametrized with the low-rank matrices $\Delta \Theta^{(i)}$ and $\widehat{Y}^{(i)} \in \mathbb{R}^{d \times b}$ are the output activations. We can express the distillation objective of the $i^{th}$ Student module as:
\begin{equation}
...
We opted for the same loss as \citet{chang2022distilhubertspeechrepresentationlearning} because we observed that the loss yields instabilities:
\begin{align}
...
Finally, we observe that the functions can be any other module in the neural network than linear layers: any sub-part of the model that outputs low-rank activations can be compressed with our method. In our experiments, we distill at the Transformer layer level and leave comparisons with other distillation levels for future work. We also experiment with other models than pretrained Transformer-based language models.
...
This approach removes $N$\% parameters from each weight matrix in the model. This approach doesn't make any assumption about the importance of different layers but is also costly because it requires computing the low-rank approximation of all weight matrices. It also requires to train all the model's layers, hence requires more GPU memory and computation as it requires forwarding and backwarding through all layers of the model.

\subsection{Top Layers First Compression}
This approach removes $N$\% parameters from the model by prioritizing the top layers. This is more efficient than the uniform one as it requires computing the low-rank approximation of fewer weight matrices and training fewer layers. However, it still requires loading the full model in GPU memory and forwarding through all the layers although only a subset of them are compressed and trained.

\subsection{Bottom Layers First Compression}
This approach removes $N$\% parameters from the model by prioritizing the bottom layers. Compared to the previous two approaches, this one is more efficient as it requires computing the low-rank approximation of fewer weight matrices and training fewer layers. It also allows to load only a subset of the model in GPU memory and forward through only a subset of the first compressed layers, which is not the case for the previous two approaches. As we will show it, this approach can compress big models that could not fit into GPU memory. The Algorithm~\ref{alg:two} describes the bottom-first compression approach.
...
\subsection{Ease of Implementation of the approach}
The approach is straightforward to implement. Figure~\ref{fig:pseudocode_impl} shows a simple pseudo-code implementation that can be easily integrated into most recent PyTorch models.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{papers/lillama/pseudo-code.pdf}
    \caption{Example of PyTorch implementation of our approach using the Teacher-only distillation strategy.}
...
\subsubsection{Data.}
For calibration, we use 13 million tokens randomly sampled from Slim-Orca \citet{SlimOrca}, an open-source replication of Orca \citet{mukherjee2023orca}. In preliminary experiments, we also tested other datasets such as RedPajama \citet{together2023redpajama}, but observed better results with instruction data.
...
We apply the Teacher+Student loss, as illustrated in Figure~\hyperref[fig:distillaton]{2c}, to all models, as we found that this loss converges faster and generally performs better. Additionally, we use the bottom-first compression strategy, which proved to be effective and less memory-intensive than other strategies.

\subsubsection{Evaluation.}
To measure inference speed, we measure the \textit{Time to First Token}, which is the time to forward a batch of prompts through the entire model and present the results in tokens per second. Using lm-evaluation-harness \citet{eval-harness}, we evaluate zero-shot performance on 9 downstream tasks: TruthfulQA (TruQA) \citet{lin-etal-2022-truthfulqa}, Social IQa (SIQA) \citet{sap2019social}, LogiQA \citet{liu2020logiqa}, WinoGrande (WinoG) \citet{sakaguchi2019winogrande}, ARC Easy (ARC-E) \citet{arc}, ARC Challenge (ARC-C) \citet{arc}, BoolQ \citet{boolq}, PIQA \citet{piqa}, and OpenBookQA (OBQA) \citet{obqa}. We compare non-compressed (0\%) and compressed variants.
...
Table~\ref{tab:mem-speed} shows memory gain (VRAM) and speedup of compressed models. We can see that 20\% compressed models are lighter and up to 20\% faster.
...
In this section, we show the generalizability of our approach by evaluating it on the Mamba architecture for text \citet{gu2024mambalineartimesequencemodeling} and on the speech modality with Whisper \citet{radford2022robustspeechrecognitionlargescale}.
We further evaluate it on a lower-resource African language in the appendix.
...
Therefore, Linear Attention Models have been proposed as alternatives to Transformers, in particular State Space Models, such as the Mamba architecture \citet{gu2024mambalineartimesequencemodeling}, which have shown promising results.

\paragraph{Method.} We tested our approach on two Mamba architectures: Mamba
3B \citet{gu2024mambalineartimesequencemodeling} (\url{https://huggingface.co/state-spaces/mamba-2.8b-hf)} and Falcon-Mamba 7B (\url{https://huggingface.co/tiiuae/falcon-mamba-7b)}.
...
We evaluate next our approach on a state-of-the-art speech model: \textit{whisper-medium.en} \citet{radford2022robustspeechrecognitionlargescale}, a 764M parameters encoder-decoder speech recognition model trained on English.

\paragraph{Method.} We take inspiration from the deep encoder,
shallow decoder approach \citet{kasai2021deepencodershallowdecoder}, where the
...
\paragraph{Data.} For calibration, we used 60\% of randomly sampled examples from Librispeech-train-100 \citet{librispeech}. We also fine-tuned the compressed model on a subset of 10 hours of speech randomly sampled from Librispeech-train-100.

\paragraph{Results.} We evaluate the models' Word Error Rate (WER), memory footprint and latency on an out-of-distribution test dataset: the test split of the English Fleurs dataset \citet{fleurs}.
...
\subsection{Which distillation loss to use?}
\begin{figure*}[h!]
    \centering
...
\begin{table}[h!]
    ...
    \bottomrule
    \end{tabular}
    \caption{\textbf{The joint loss generally performs better}: Zero-shot performances of the three losses using the same setup for each model, as described in Section~\ref{sec:xp}.}
    \label{tab:losses}
\end{table}

Table~\ref{tab:losses} shows that the joint loss depicted in Figure~\hyperref[fig:distillaton]{2c} generally produces the best results. Although the differences between the Student and Teacher+Student losses may not seem significant, Figure~\ref{fig:losses} shows that the Teacher+Student loss leads to faster convergence than the Student loss alone. It also generally results in a lower final perplexity than using the Teacher loss only.
...
\subsection{How to choose the rank?}
In our bottom-first compression strategy, a small rank in
Algorithm compresses more severely the bottom layers and leaves
more upper layers uncompressed, hence saving GPU memory. However, a
small may also destroy knowledge and the model will struggle to recover
...
\paragraph{Differences in performances.}
...
\bottomrule
\end{tabular}
\caption{The Civil War Story by the 20\% and 40\% compressed models, without \textit{any} finetuning. \textbf{Prompt: "What is the cause of the Civil War? Here is the story."}}
\label{tab:cw}
\end{table*}

\subsection{Comparisons.}
Table~\ref{tab:comparisons} shows that our approach outperforms SliceGPT \citet{ashkboos_slicegpt_2024}, a one-shot compression method for LLMs. Our approach also compares favorably with SVD-LLM \citet{svdllm} in Section \ref{ssec:svdllm}.

\subsection{Can this approach work for low-resource and under-represented languages?}
\label{sec:hausa-ch2}

For most human languages, there is not enough data (generally less than 1B tokens) to pretrain large LLMs. In such cases, data-efficient approaches are preferred. We experimented compressing a tiny language model for Hausa, a low-resource language for which there is not billions of tokens available for continued pretraining. We used InkubaLM~\citep{tonja2024inkubalm}, a 422M parameter language model designed for five low-resource languages. Since 60\% of the parameters are in the embeddings, we focused on compressing the input embedding $W_{e}$ and the prediction head $W_{o}$. This is easily achieved with our approach by locally distilling the low-rank embeddings and prediction head:
...
We used a rank of 1024, compressing the model by 30\%. Then, we applied a lightweight local distillation using our approach, with 64k randomly sampled Hausa sentences from InkubaMono\footnote{\url{https://huggingface.co/datasets/lelapa/Inkuba-Mono}} and the Aya-Dataset~\citep{singh2024aya}. We compared the models with MobiLLaMA~\citep{thawakar2024mobillama} and SmoLLM\footnote{\url{https://huggingface.co/blog/smollm}} using the Afrimmlu benchmark~\citep{afrimmlu}. As shown in Table~\ref{tab:inkuba-ch2}, the compressed InkubaLM model for Hausa retains 93\% of its base performance.
...
\subsection{Compromise between specificity and genericity:}
Efficient LLM compression requires trade-offs between speed and generality. We prioritize generality over raw speed, avoiding GPU-specific kernels as used in semi-structured sparsity approaches. Efficiency also ties to carbon cost: our method works on CPU as well as on
...
\subsection{If the weights are full rank, why still compressing them?}
Our main approach to distill activations in compressed weights is based on the observation that activations are low-rank while the weights are full-rank. However, we still compress the weights and not the activations. The fact that activations are low-rank should be a motivation to compress them. Later we will discuss activations compression methods such as Matryoshka Representatin Learning (MRL) and activations sparsity.
