...
\chapter[Adaptive Sparse Memory]{Adaptive Sparse Memory Transformer}\label{ch:zon}
...
The question is how to train large models efficiently at smaller computational cost.
...
memory layers communication-bound, eespecially when the memory spans many GPUs.
...
\begin{enumerate}
    ...
    spaces which occupy more fully this small vectorial space. These observations
    ...
    methods to learn activation sparsity in a differentiable almost everywhere way.
    ...
    its local memory slots without needing to dispatch token representation as in TopK-based routing.
...
\begin{table}[!htbp]
    ...
    MoE layers receive different number of tokens.
    We look to the correlation between the number of routed tokens
    per expert and the energy ratio $R@0.99$ and have foun none.}
...
dimension per expert and $d_{\text{inter}}$ is the total interemediate dimension
of the model.}
...
This motivates a large intermediate dimension but only selecting individual memory slots for each input token.
...
Recalling the MLP definition as key-value memory, its sparsely activated version can be written as:
\begin{align}
...
constraint as it gives more flexibility. Hence we define
the average expected number of active gates across all MLP layers $L$ as
$\mathbb{E}_{\text{L}}\!\left[\|\mathbf{Z}\|_0\right] = \frac{1}{L} \sum_{l=1}^{L}\mathbb{E}_{l}\!\left[\|\mathbf{z}\|_0\right].$
Putting $\mathbb{E}_{\text{L}}\!\left[\|\mathbf{Z}\|_0\right]$ in Equation~\ref{eq:zon-lg}
...
is free and can be made complex, non-linear and deep. However,
...
inference computation relative to dense models.

First, the approach is a trainable structured activation sparsity. The sparsity
...
during the MLP forward. There are many sparse GPU kernels in the literature
that can be adapted to accelerate training and inference, such as DejaVU or TEAL.
...
distributed training, especially in tensor parallelism sharding.
...
sparse memory layer that needs to dispatch token representations to the
...
for dense training. For fair comparisons, we keep unchanged the hyperparameter
settings, which have been optimized for dense models. Transformer matrices,
...
The gates are initialized to keep almost all coordinates active at the beginning of the training.
The gradient signal of the sparsity loss can be as strong as the language modeling
...
ZoN-1.2B-A600M a 1.2B dense model with 600M active parameters using our proposed approach.
...
approximate active parameter count. This setup helps us study whether at same
...
input. Learned adpative activation sparsity allow the amount of sparsity to depend on the input. 
