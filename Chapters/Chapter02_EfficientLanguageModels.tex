...
budget. We discuss here LLM efficiency mainly though the lens of \textbf{model compression} and \textbf{activation sparsity},
since our contributions concern these two aspects. 
We present here common approaches to make LLMs efficient to scale during training, 
but also to serve efficiently during inference.
...
LLMs such LlaDa or Gemma-Diffusion are much faster than autoregressive LLMs.
Diffusion LLMs are compute-bound such that they are more FLOP-Intensive than autoregressive LLMs.
So, since the decoding is parallel it reduces the data-movement compared to autoregressive LLMs,
...
\subsection{Layer Looping: Increasing expressivity without increasing model parameters}
...
estimate the importance and removes the least important groups. The method can
...
of the pruned model compared to from scratch training. Hence, pruning larger models then
continued pre-training can be a good approach for creating small models than training
...
\end{figure}
\subsubsection{Co-Distillation}
\section{Activation Sparsity}
While post-training compression of LLMs has proved to be an effective approach
to create small LLMs, larger LLMs nevertheless remain largely more performant.
...
but with a cheaper training and inference cost? Sparse activation is one answer
...
sparse Mixture-of-Experts, which is now the most successful class of sparse models at
...
In a sparse MoE, the weight matrix $\mathbf{W}$ is split into $e$ subweights called experts,
$\mathbf{W}_1, \ldots, \mathbf{W}_e$. The model is trained to decides which
experts are useful for the current input $\mathbf{x}$, and only computes these
...
first applied on these router logits to select the subset of active experts,
...
using a projector weight $\textbf{W}_o \in \mathbb{R}^{d_e \times d_2}$.
...
the computation by the same factor. Switch Transformer~\citep{switch} shows that at FLOP-matched
...
On the left plot, we can see that keeping the number of activated paramerers fixed,
while increasing the number of experts, leads to strong improvement. However,
it doesn't seem linear, and at some point increasing the number of parameters
without increasing the number of activated parameters stop being beneficial.
And since biffer models converges faster, large MoEs accelerate also the training
as shown by the left plot.
...
\begin{figure}[!ht]
    ...
    \caption{Scaling properties of Switch Transformer. Increasing the number of experts improves performance and sample efficiency at equal computational budget. Figure from \citet{switch}.}
    \label{fig:switch}
\end{figure}
The sparsity has been pushed much farther in recent sparse MoEs. The DeepSeek
...
token, which represents approximately $97\%$ of sparsity~\citep{deepseekai2024deepseekv3}. 
DeepSeek-V4-Pro pushes this even further: $384$ routed experts and activates only $6$ per
token, so more than $98\%$ of the routed experts are inactive at each
step~\citep{deepseekai2026deepseekv4}. Since MoEs decouple parameters
...
In \citet{krajewski2024scaling}, the authors study this effect while keeping the
...
probably because it gives to the router more flexibility to combine experts. 
...
\begin{figure}[!ht]
    ...
    \caption{Effect of MoE granularity on loss for fixed model size and number of training tokens. Figure adapted from \citet{krajewski2024scaling}.}
    \label{fig:fine-grained-moe-granularity}
\end{figure}

\subsubsection{Stabilizing MoEs}
While pushing large, sparse and granular MoEs results in strong efficient models,
it also results in challenging training. This is true in terms of infrastructure,
since more experts creates a communication overhead between nodes and GPUs, and
irregular GPUs memory loading when some experts are used more than other.
But it is also true in terms of optimization dynamics, since the topk-based router
is not a differentiable operation, it hurts the optimization dynamics and require
a lot of tricks to optimize training.
...
overloaded, while the other experts are almost not trained. Their hosting GPUs
...
\paragraph{Waking the deads.} Even with load balancing constraints,
...
A recent blogpost\footnote{
    \href{https://alltoall.notion.site/save-lower-layer-moe-experts-llal}{https://alltoall.notion.site/save-lower-layer-moe-experts-llal}
...
temporarily compute the language modeling based on the output of the collapsing MoE layers, 
so they are "forced" to become useful.

\subsubsection{Are "experts" really experts?}
There is a debate about the possibility of experts to specialize in some
domain. So far, there is no clear observed \textbf{expert specialization} in MoE LLMs,
...
were domain experts, we would expect some experts to be ofted selected
for some domains. But this is not what happen. As shown in
Figure~\ref{fig:mixtral-expert-selection-domains}, the routing to different domains is
...
\begin{figure}[!ht]
    ...
    Figure adapted from \citet{jiang_mixtral_2024}.}
    \label{fig:mixtral-expert-selection-domains}
\end{figure}

However, expert specialization can be more salient on surface forms, such as
...
not necessarily a clean semantic specialization. This finding is somehow
validated in OLMoE, where the authors observe a strong vocabulary-level
...
\begin{figure}[!ht]
    ...
    in NLLB-200. Figure adapted from \citet{nllbteam2022nolanguageleftbehind}.}
...
\begin{table}[!ht]
    ...
    Table adapted from \citet{muennighoff2025olmoe}.}
...
``Parameters is knowledge, FLOPs is intelligence.''\footnote{
    \href{https://x.com/LiamFedus/status/2090363702042304847?s=20}
    {\nolinkurl{https://x.com/LiamFedus/status/}\allowbreak\nolinkurl{2090363702042304847?s=20}}
}
...
be weaker on tasks where the bottleneck is more compute than more memorized
...
This intuition is now well shared in the field, and many works propose ways
to make MoE routing compute-adaptive\footnote{
    See this blogpost for an overview of the approaches
    \href{https://huggingface.co/blog/Spico/dynamic-routing}{https://huggingface.co/blog/Spico/dynamic-routing}
}. \citet{huang2024hardertasksneedexperts} propose a routing strategy inspired by
TopP decoding. Contrary to TopK, TopP does not activate a fixed number of experts.
...
training with a regularization on these scores. The number of active experts is
...
This approach is for example used in LongCat-Flash
...
\begin{figure}[tb]
    ...
    Figure from \citet{meituanlongcatteam2025longcatflashtechnicalreport}.}
...
The idea to augment neural networks with a large memory is not new \citep{weston2015memorynetworks},
but in the context of LLMs it has been systemized by
\citet{lample2019largememorylayersproduct}, where the approach is made
efficient and scalable to modern LLM scale.
...
\begin{figure}[tb]
    ...
    the selected keys are used for computation. Figure from \citet{lample2019largememorylayersproduct}.}
    \label{fig:key-value-memory-layer}
\end{figure}

Using the notation of \citet{lample2019largememorylayersproduct},
...
\subsubsection{Scaling Memory Layer}
\paragraph{Product-Key Memory}
Product-Key Memory (PKM) \citet{lample2019largememorylayersproduct} 
makes the keys search scalable by using a product structure.
...
fixed. The model can access to more smaller values, and compose a richer output.
Second, they observe that PKM is \textbf{memory-bound} during trainnig. A very large
sparse memory layer is cheap in FLOPS, but it can be expensive in memory access and
...
Expert Retrieval), proposed by \citet{he2024mixturemillionexperts}. Conceptually,
...
following the pseudo-code of \citet{he2024mixturemillionexperts}.
...
\begin{figure}[tb]
    ...
    Figure from \citet{liu2026scalingembeddingsoutperformsscaling}.}
    \label{fig:ngram-embedding-layer}
\end{figure}
Some solutions exist to augment the LLM memory without the complexity
and cost of searching in a large key-value store. 
Embedding memory layers can be seen as a special case of sparse key-value memory
layers, where the keys are not learned dense vectors but fixed discrete indexes.
...
layers.\footnote{
    ...
    {\nolinkurl{https://github.com/KellerJordan/modded-nanogpt}}
}
...
LongCat-Flash~\citep{liu2026scalingembeddingsoutperformsscaling} or Qwen3.8-Flash-Next
\footnote{
    \href{https://github.com/QwenLM/Qwen3.8-Flash-Next/blob/main/tech_report.pdf}
    {\nolinkurl{https://github.com/QwenLM/Qwen3.8-Flash-Next/blob/main/tech_report.pdf}}
}
Figure~\ref{fig:ngram-embedding-layer} shows how N-gram Embedding is used in
...
uses it to construct the input token representations to the Transformer backbone.
Qwen3.8-Flash-Next instead injects the retrieved n-gram vectors inside the
Transformer, at a selected layer. 
The authors find that no single layer position
They also found that when the n-gram vocabulary is
...
ReLU-like activations. This process is called \textbf{ReLUfication}. In gated
...
Another line of approach is \textbf{Matryoshka Learning}, where a single model is
...
$d$-dimensional representation. Most of time, this function is simply a slicing operation
that selects the first $d$ dimensions of the representation, but it can also be a learned projection. 
MRL framework is illustrated in Figure~\ref{fig:mrl-figure1} and
is used in a lot in retrieval models. It helps reduce the disk usage of the embedding
store, and allows a trade off between retrieval speed and accuracy.
MRL can be seen as an activation sparsity since slicing a vector $x_{[:d]}$
...
\begin{figure}[!ht]
    ...
    useful at several embedding dimensions. Figure adapted from \citet{MRL}.}
...
Gemma~3n E2B and E4B\footnote{\href{https://developers.googleblog.com/en/introducing-gemma-3n-developer-guide/}{\texttt{https://developers.\allowbreak googleblog.com/\allowbreak en/\allowbreak introducing-\allowbreak gemma-3n-\allowbreak developer-\allowbreak guide/}}.}
and Gemma~4 E2B and E4B.\footnote{\href{https://ai.google.dev/gemma/docs/core/model_card_4}{\texttt{https://ai.\allowbreak google.dev/\allowbreak gemma/\allowbreak docs/\allowbreak core/\allowbreak model\_card\_4}}.}
