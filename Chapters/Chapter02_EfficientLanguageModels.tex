...
budget. We discuss in this Chapter LLM efficiency mainly though the lens of \textbf{model compression} and \textbf{activation sparsity},
since our contributions concern these two aspects. 
We present common approaches to make LLMs efficient to scale during training, 
...
\textbf{MFU}(measures how much of the theoretical peak compute of the hardware is really used),
...
We detail in the next sections some approaches to make LLMs efficient on the key metrics we presented,
both inference and tranining. This part is heavily inspired by the blogpost we wrote in collaboration 
...
which consists of training a large model first, then compress it by keeping the most important components of the model, 
...
were among the first to strongly push the idea that
training should also account for inference compute. Instead of only following what
Chinchilla compute-optimality suggests \citep{hoffmann2022training}, they train small models on much more
...
resources. The release Llama~1 \citep{touvron2023llamaopenefficientfoundation} 
...
More importantly, the Llama~3 advocates training small models much longer, beyond what is compute-optimal.
...
While this is costly at training time, the resulting small models are stronger and efficient to serve for
inference.
...
large models first and then compress them or distill them in efficient architecture. 
...
than structured pruning. However, structured pruning has more potential
in improving inference and reducing memory footprint.
...
but there many other objectives.
...
\end{figure}
\begin{insightbox}[breakable]{Our contributions to LLM Compression}
In the domain of compression domain we make two contributions that compress pre-trained Transformers:
\begin{enumerate}
    \item We explore low-rank decomposition for compressing pretrained LLMs
    and propose \textbf{Lillama}, a method that combines low-rank weight
    approximation with layer-wise distillation. We address the \textbf{training--inference mismatch} 
    by making layer-wise distillation more \textbf{on-policy}: each student layer 
    is trained on activations produced by preceding student layers.

    \item Post-training compression often leaves the LM head untouched,
    as compressing it can distort the output probability distribution
    and require retraining to recover performance. We propose
    \textbf{BaldWhisper}, showing that low-rank compression of Whisper's LM head, 
    combined with feature distillation, offers a good speed--accuracy trade-off 
    despite the constraints of the \textbf{softmax bottleneck}. 
    We also explore \textbf{layer merging} as an alternative to layer
    pruning, reducing decoder depth to lower self-attention and
    cross-attention costs during autoregressive decoding, which is
    \textbf{memory-bounded}.
\end{enumerate}
\end{insightbox}
...
experts are useful for the current input $\mathbf{x}$, and only compute these
...
And since bigger models converges faster, large MoEs accelerate also the training
...
many tricks to optimize training.
...
They found that when the n-gram vocabulary is
...
\begin{insightbox}[breakable]{Our contributions to Activation Sparsity}
...
\begin{enumerate}
    ...
    in relatively few components leaving the remaining unused. 
    ...
    differentiable sparsity mechanism for both MLP activations
    and embedding representations.
\end{enumerate}
\end{insightbox}
